...
const { homeName, awayName, homeScore, awayScore, stadium, date, matchType} = matchData || {}
  return (
    <div className="flex  min-w-[80vw] md:min-w-[40vw] mx-auto h-[10rem] text-gray-300 flex-col teams-card rounded-md px-4 font-poppins py-2 pb-4"  onClick={()=>handleMatchCard(id)}>
      ...
      <div className="flex flex-1 items-center">
        <div className="flex items-center w-2/5 flex-col flex-2">
          ...
          <p className="text-[.7rem] text-center">{homeName}</p>
        </div>
        <div className="1/5 flex-1 text-center flex flex-col">
        {  fixture? <div>vs</div> : <div><span>{homeScore}</span> : <span>{awayScore}</span></div>}
        ...
        <div className="flex items-center w-2/5 flex-col text-center">
          ...
          <p className="text-[.7rem] text-center">{awayName}</p>
        </div>
      </div>
      <div className="flex text-[.7rem]">
        ...
        <div className="ml-auto flex items-center gap-2"><span className="text-[1.2rem]"><MdCalendarMonth /></span>{date}</div>
      </div>
    </div>
  );
  }
