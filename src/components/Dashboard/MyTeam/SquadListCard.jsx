...
const PlayerNumber = ({ num }) => {
  ...
  export default function SquadListCard({ playerData, openDialog,  openEditPlayerDialog }) {
    ...
    return (
      <div className={`flex relative  ${isRemoved? "scale-[0]" : "scale-[1]"} transition-transform duration-1000 text-[.8rem] sm:text-[1.2rem] py-2 font-poppins items-center border-b border-gray-900 lg:hover:bg-gray-500 cursor-default`}>
        ...
        <div className="flex-1 flex text-[1.3rem] md:text-[1.6rem]  justify-evenly">
          <button type="button" className="text-primary lg:hover:text-inherit" onClick={()=>openEditPlayerDialog(id)}>
            <AiOutlineEdit />
          </button>
          <button
            type="button"
